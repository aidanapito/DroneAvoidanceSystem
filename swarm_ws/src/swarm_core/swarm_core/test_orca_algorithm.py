...
class DroneSimulator:
    ...
    def get_odometry_msg(self, clock) -> Odometry:
...
class ORCATestNode(Node):
    ...
    def __init__(self):
        ...
        # QoS profile matching ORCA node
        fast_qos = QoSProfile(
        ...
        self.safe_velocities_sub = self.create_subscription(
            Twist, '/swarm/safe_velocities', self._safe_velocities_callback, fast_qos
        )
        
        self.collision_risk_sub = self.create_subscription(
            Bool, '/swarm/collision_risk', self._collision_risk_callback, fast_qos
        )
        
        self.min_separation_sub = self.create_subscription(
            Float32, '/swarm/min_separation', self._min_separation_callback, fast_qos
        )
    ...
    def _simulation_timer_callback(self):
        ...
        for drone_id, drone in self.drones.items():
            ...
            odom_msg = drone.get_odometry_msg(self.get_clock())
